...
import json
from flask_ask import Ask, request, session, question, statemen
...
def areacurve(a,b,c,d,e):
    ...
    print (myfunction)
@ask.intent("Maximum_between_two_dates")
def run_query(start_date, end_date, companyname, function):
...
def get_ticker(start_date, end_date, companyname, function):
    ticker = Quack.getdict(companyname)
    if function == 'average':
        calculated_data = Quack.avg_shares(ticker, start_date, end_date)
        return question("The " + function + " shares price between" + start_date " and " + end_date + " is " + calculated_data)
    else if function == 'minimum':
        calculated_data = Quack.min_shares(ticker, start_date, end_date)
        return question("The " + function + " shares price between" + start_date " and " + end_date + " is " + calculated_data)
    else if function == 'maximum':
        calculated_data = Quack.max_shares(ticker, start_date, end_date)
        return question("The " + function + " shares price between" + start_date " and " + end_date + " is " + calculated_data)
    else:
